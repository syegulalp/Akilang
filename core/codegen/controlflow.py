...
class ControlFlow:
    ...
    def _check_array_return_type_compatibility(self, returnval, returntype):
        try:
            ...
            if t0.arr_type == t1.arr_type:
                new_returnval = self.builder.bitcast(returnval, returntype)
    ...
    def _codegen_Return(self, node):
        ...
        if self.func_returntype is None:
            raise CodegenError(f"Unknown return declaration error", node.position)
    ...
    def _codegen_Match(self, node):
        ...
        for value, expr in node.match_list:
            ...
            if val_codegen.constant in values:
                raise CodegenError(
                    f'Match parameter "{value}" duplicated', value.position
                )
    ...
    def _stacksave(self, node):
        stacksave = self.module.declare_intrinsic(
            "llvm.stacksave", [], ir.FunctionType(self.vartypes.u_mem.as_pointer(), [])
        )

        result = self.builder.call(stacksave, [])
        return result
    ...
    def _stackrestore(self, node, restore_point):
        ...
        result = self.builder.call(stackrestore, [restore_point])
        return result
    ...
    def _codegen_Loop(self, node):
        ...
        if node.start_expr is None:
            ...
            self.loop_exit.append((loopafter_bb, loopbody_bb))
            ...
            self.loop_exit.append((loopafter_bb, loopcounter_bb))
        ...
        if var_addr is None:
            ...
            self._codegen_Assignment(node.start_expr, node.start_expr.initializer)
    ...
    def _codegen_Continue(self, node):
        ...
        if not self.loop_exit:
            raise CodegenError('"continue" called outside of loop', node.position)
    ...
    def _codegen_With(self, node):
        ...
        for n in reversed(new_bindings):
            self._autodispose([[n, self.func_symtab[n]]], node)
