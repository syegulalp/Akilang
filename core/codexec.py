...
class AkilangEvaluator(object):
    ...
    def load_file(self, f):
        try:
            ...
            print(colored(f"Could not load basic library: {err}", "red"), f)
            self._reset_base()
            raise err

    def reset(self, history=[], force=False):
        self._reset_base(force=force)
        if self.cached_lib:
        ...
        if self.basiclib_dir:
            ...
            self.load_file(
                os.path.join(self.basiclib_dir, "platform", os.name, "platformlib.aki")
            )

            # Finally, load the non-platform dependent basiclib
            self.load_file(os.path.join(self.basiclib_dir, self.basiclib_file))
    ...
    def _reset_base(self, module_name=None, force=False):
        self.codegen = LLVMCodeGenerator(module_name=module_name, force=force)
        self.vartypes = self.codegen.vartypes
    ...
    def eval_generator(self, codestr, options=dict()):
        ...
        for ast in Parser(
            self.codegen.module, anon_vartype=anon_vartype, vartypes=self.vartypes
        ).parse_generator(codestr):
    ...
    def _eval_ast(
        ...
        if llvmdump:
            dump(
                str(self.codegen.module), f'{paths["dump_dir"]}\\__dump__unoptimized.ll'
            )
        ...
        def_or_extern = not (
            (isinstance(ast, Function) and ast.is_anonymous())
        ...
        if optimize:
            ...
            if llvmdump:
                dump(str(llvmmod), f'{paths["dump_dir"]}\\__dump__optimized.ll')
        ...
        target_machine = self.target.create_target_machine(opt=3, codemodel="large")
        with llvm.create_mcjit_compiler(llvmmod, target_machine) as ee:
            ee.finalize_object()
            ...
            if llvmdump:
                ...
                print(
                    colored(
                        f'Code dumped in local directory {paths["dump_dir"]}', "yellow"
                    )
                )
    ...
    def eval_and_return(self, node):
        ...
        # for this constant
        self.codegen.generate_code(Function.Anonymous(node.position, node))
        ...
        e = self._eval_ast(
            Function.Anonymous(node.position, node, vartype=f_type), return_type=r_type
        )
